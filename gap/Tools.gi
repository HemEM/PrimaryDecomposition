...
InstallMethod( RepresentationOverCoefficientsRing,
  ...
  function( r )
    ...
    for i in [ 1 .. d ] do
        ...
        Perform( [ 1 .. Length( pos ) ], function( j ) \
                 SetMatElm( mat, i, pos[j], MatElm( coeffs, j, 1 ) / k ); end );
    od;
...
InstallMethod( BasisCoefficientsOfRingElement, 
  ...
  function( r )
    ...
    Perform( [ 1 .. Length( pos ) ], function( j ) SetMatElm( mat, 1, pos[j], \
             MatElm( coeffs, j, 1 ) / k ); end );
...
##
InstallMethod( IdealBasisOverCoefficientsRing,
        [ IsHomalgMatrix ],
  ...
  function( G )
    ...
    W := List( [ 1 .. NrRows( G ) ], i -> BasisCoefficientsOfRingElement( 
               MatElm( G, i, 1 ) / R ) );
    ...
    return Iterated( List( [ 1 .. NrRows( W ) ],
                     i ->  R * CertainRows( W, [ i ] ) * bas ), UnionOfRows );
...
InstallMethod( IdealBasisToGroebner, 
  ...
  function( M )
    ...
    #!  We compute reduced echelon fomrs using GAP and homalg which does not support
...
InstallMethod( AppendToGroebnerBasisOfZeroDimensionalIdeal,
  ...
  function( G )
    ...
    if not HasRationalParameters( CoefficientsRing( R ) ) then
        return IdealBasisToGroebner( IdealBasisOverCoefficientsRing( G ) );
    fi;
  ...
  function( f )
    ...
    for i in [ 1 .. NrRows( coeffs ) ] do
        ...
        if not IsOne( monoms[i] ) then
            ...
            f := f + MatElm( coeffs, i, 1) * Degree( monoms[i] ) * indets[1]^ \
                 ( Degree( monoms[i] ) - 1 );
...
InstallMethod( GeneratorOfAnElementNotContainedInAnyHyperplane,
  ...
  function( L )
    ...
    while true do
        ...
        Perform( [ 1 .. NrColumns( L ) ], function(j) SetMatElm( lambda, 1, j , \
                 Random( rs, [ - 50 .. 50 ] ) / R ); end );
