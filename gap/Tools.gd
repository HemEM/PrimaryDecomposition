...
#!  univariate polynomial <A>p</A> by computing ideal generated 
...
#!  <A>G</A> over <M>R</M>, where <M>K</M> is the coefficients ring of <M>R</M>,
#!  using an algorithm of Sebastian Jambor (see <Cite Key="SJ"/>).
DeclareOperation( "IdealBasisOverCoefficientsRing", 
	[ IsHomalgMatrix ] );
...
#!  The basis elements are given as the rows of the matrix <A>M</A> in form
...
DeclareAttribute( "IdealBasisToGroebner", 
	IsHomalgMatrix);
#! @InsertSystem IdealBasisToGroebner
